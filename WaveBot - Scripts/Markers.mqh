...
// Optional extra namespace for parallel worlds (e.g., "MAJ").
// Empty => legacy behavior (no extra namespace).
static string g_markers_ns = "";

// Preview mode is used by Central Multi-Symbol non-primary
// passes. Preview suppresses visuals but market-state calculations and
// hunter event clocks continue independently for each symbol.
static bool   g_markers_preview_mode = false;

// set/get chart-analysis namespace
...
// One-time upgrade cleanup: remove chart objects left by older EA builds.
// There is no post-MTC detection, marker registration, or zone tracking here.
inline void Markers_DeleteObsoleteObjects()
{
   const int total = ObjectsTotal(0);
   for(int i = total - 1; i >= 0; --i)
   {
      const string name = ObjectName(0, i);
      if(StringLen(name) < 4 || StringSubstr(name, 0, 1) != "S")
         continue;

      // Match only legacy WaveBot annotation names (including MAJ namespace).
      if(StringFind(name, "M15_NEW_AFTER_MTC_") >= 0 ||
         StringFind(name, "M15_NEW_ZONE_") >= 0)
         ObjectDelete(0, name);
   }
}
