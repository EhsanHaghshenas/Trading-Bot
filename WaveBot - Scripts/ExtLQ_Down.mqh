...
// -------------------- Context برای چند دنیا (ماژور / مینور) --------------------
struct ExtLQDownContext
{
   bool      ext_has;      // همان g_extD_has
   double    ext_price;    // همان g_extD_price
   datetime  ext_time;     // همان g_extD_time
   LQLevel_D hist[];       // کپی از g_histD[]
   int       prev_idx;     // همان g_prev_idxD
};

// مقداردهی اولیهٔ یک کانتکست خالی
inline void ExtLQ_Down_ContextInit(ExtLQDownContext &ctx)
{
   ctx.ext_has   = false;
   ctx.ext_price = 0.0;
   ctx.ext_time  = 0;
...
// ریست کامل وضعیت داخلی ماژول (برای شروع یک دنیای جدید)
inline void ExtLQ_Down_ResetGlobals()
{
   g_extD_has   = false;
   g_extD_price = 0.0;
   g_extD_time  = 0;
...
inline int ExtLQ_Down_FindLatestUntouchedIndex()
{
   int sz = ArraySize(g_histD);
...
inline void ExtLQ_Down_Set(const double price, const datetime t)
{
   ...
   // خط فعلی
   if(InpDrawExtLQ)
      ExtLQ_Down_DrawLine(EXTLQ_D_LINE_CURR, price, InpExtLQColor, STYLE_SOLID);
